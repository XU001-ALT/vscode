...
import type { ChartRecommendation, ChartType, Lang, SqlCorrection } from '../types'
import { t } from '../i18n'
...
export default function ChartView({ columns, rows, recommendation, lang, sql, corrections }: Props) {
  ...
  return (
    <div>
      ...
      <div className="result-actions">
        <button className="btn-outline" onClick={() => setShowTable(s => !s)}>
          {showTable ? t('back_chart', lang) : t('data_table', lang)}
        </button>
        <button className="btn-outline" onClick={resetControls}>{t('reset', lang)}</button>
        <button className="btn-outline" onClick={downloadPng} disabled={showTable}>{t('export_png', lang)}</button>
      </div>
          ...
          <table className="data-table">
            ...
            <tbody>
              {filteredRows.map((r, i) => (
                <tr key={i}>{r.map((v, j) => <td key={j}>{formatCell(v)}</td>)}</tr>
              ))}
            </tbody>
          </table>
        </div>
      ...
      {filteredRows.length < rows.length && (
        <div className="filter-count">{t('rows_returned', lang)}{filteredRows.length}{t('rows_unit', lang)}</div>
      )}
      </>
      )}

      {(sql || corrections?.length) && (
        <details className="sql-box">
          <summary>{t('view_sql', lang)}</summary>
          {sql && <pre className="sql-code">{sql}</pre>}
          <div className="sql-box-sub">
            <div className="sql-sub-title">{t('corrections', lang)}</div>
            {corrections && corrections.length > 0 ? (
              corrections.map(c => (
                <div key={c.attempt} className="corr-item">
                  <div className="corr-attempt">#{c.attempt}</div>
                  <div className="corr-line"><span className="corr-label err">{t('failed_sql', lang)}</span><code>{c.failed_sql}</code></div>
                  <div className="corr-line"><span className="corr-label err">{c.error}</span></div>
                  <div className="corr-line"><span className="corr-label ok">{t('fixed_sql', lang)}</span><code>{c.fixed_sql}</code></div>
                </div>
              ))
            ) : (
              <span className="hint-ok">{t('no_corrections', lang)}</span>
            )}
          </div>
        </details>
      )}
